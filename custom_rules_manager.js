...
<meta name="savepage-url" content="https://ejjrmqjx.manus.space/js/custom_rules_manager.js">
<meta name="savepage-title" content="">
<meta name="savepage-pubdate" content="Unknown">
<meta name="savepage-from" content="https://ejjrmqjx.manus.space/js/custom_rules_manager.js">
<meta name="savepage-date" content="Sat Apr 26 2025 13:37:00 GMT-0300 (Brasilia Standard Time)">
 ...
 * Oracle EPM Data Generator - Custom Rules Manager
 * 
 * This module provides functionality for creating, editing, and managing custom validation rules
 * that can be used alongside the built-in Oracle EPM best practices rules.
 * 
 * Features:
 * - Create custom rules for different categories (naming, data validation, etc.)
 * - Edit existing custom rules
 * - Enable/disable custom rules
 * - Store custom rules in browser localStorage
 * - Export/import custom rules as JSON
 */

// Main custom rules manager
const CustomRulesManager = {
  // Storage key for localStorage
  STORAGE_KEY: 'oracleEpmCustomRules',
  ...
  /**
   * Initialize the custom rules manager
   */
  init: function() {
    this.loadRules();
    
    // Register event listeners for the custom rules UI
  ...
  loadRules: function() {
    try {
      ...
      this.customRules = storedRules ? JSON.parse(storedRules) : this.defaultRules;
    } catch (e) {
      console.error('Error loading custom rules:', e);
      this.customRules = this.defaultRules;
    }
  ...
  saveRules: function() {
    try {
      localStorage.setItem(this.STORAGE_KEY, JSON.stringify(this.customRules));
      return true;
    } catch (e) {
   ...
   * @param {string} category - Rule category (naming, dataValidation, etc.)
   * @param {string} subcategory - Rule subcategory (account, period, etc.)
   * @param {object} rule - Rule definition
   * @returns {boolean} Success status
   */
  addRule: function(category, subcategory, rule) {
    try {
      // Validate rule structure
      if (!this.validateRuleStructure(category, rule)) {
        return false;
      }
      
      // Add unique ID to rule
      rule.id = this.generateRuleId();
      
      // Add rule to appropriate category/subcategory
      if (subcategory) {
        if (!this.customRules[category][subcategory]) {
          this.customRules[category][subcategory] = [];
        }
        this.customRules[category][subcategory].push(rule);
      } else {
        if (!this.customRules[category]) {
          this.customRules[category] = [];
        }
        this.customRules[category].push(rule);
      }
      ...
      console.error('Error adding custom rule:', e);
      return false;
    }
  },
   ...
   * @param {string} subcategory - Rule subcategory
   * @param {string} ruleId - Rule ID
   * @param {object} updatedRule - Updated rule definition
   * @returns {boolean} Success status
   */
  updateRule: function(category, subcategory, ruleId, updatedRule) {
    try {
      // Validate rule structure
      if (!this.validateRuleStructure(category, updatedRule)) {
        return false;
      }
      
      // Preserve the original rule ID
      ...
      if (subcategory) {
        const rules = this.customRules[category][subcategory];
        ...
        const rules = this.customRules[category];
        for (let i = 0; i &lt; rules.length; i++) {
          if (rules[i].id === ruleId) {
            rules[i] = updatedRule;
            found = true;
      ...
      console.error('Error updating custom rule:', e);
      return false;
    }
  },
   ...
   * @returns {boolean} Success status
   */
  deleteRule: function(category, subcategory, ruleId) {
    try {
      let found = false;
      ...
      console.error('Error deleting custom rule:', e);
      return false;
    }
  },
   ...
   * @param {string} category - Rule category
   * @param {string} subcategory - Rule subcategory (optional)
   * @returns {array} Array of rules
   */
  getRules: function(category, subcategory) {
    try {
      if (subcategory) {
        return this.customRules[category][subcategory] || [];
      } else {
        return this.customRules[category] || [];
      }
    } catch (e) {
      console.error('Error getting custom rules:', e);
      return [];
    }
  },
  
  /**
   * Export custom rules as JSON string
   * @returns {string} JSON string of custom rules
   */
  ...
  /**
   * Import custom rules from JSON string
   * @param {string} jsonRules - JSON string of custom rules
   * @returns {boolean} Success status
   */
  importRules: function(jsonRules) {
    try {
      const parsedRules = JSON.parse(jsonRules);
      
      // Validate overall structure
      if (!this.validateRulesStructure(parsedRules)) {
        return false;
      }
      
      this.customRules = parsedRules;
      this.saveRules();
      return true;
    } catch (e) {
      console.error('Error importing custom rules:', e);
      return false;
    }
  },
  ...
  clearRules: function() {
    try {
      ...
      console.error('Error clearing custom rules:', e);
      return false;
    }
  },
   ...
   */
  validateRuleStructure: function(category, rule) {
    // Common required fields for all rules
    ...
    switch (category) {
      case 'namingRules':
        return typeof rule.validation === 'function' || 
               (typeof rule.validation === 'string' &amp;&amp; this.isValidFunctionString(rule.validation));
        
      case 'dataValidationRules':
        return (typeof rule.condition === 'function' || 
                (typeof rule.condition === 'string' &amp;&amp; this.isValidFunctionString(rule.condition))) &amp;&amp;
               (typeof rule.validation === 'function' || 
                (typeof rule.validation === 'string' &amp;&amp; this.isValidFunctionString(rule.validation)));
        
      case 'dimensionalHierarchyRules':
      case 'intercompanyRules':
      case 'movementRules':
        return typeof rule.validation === 'function' || 
               (typeof rule.validation === 'string' &amp;&amp; this.isValidFunctionString(rule.validation));
  ...
  /**
   * Validate the overall structure of imported rules
   * @param {object} rules - Rules object to validate
   * @returns {boolean} Validation result
   */
  validateRulesStructure: function(rules) {
    ...
    // Check dataValidationRules subcategories
    const dataValidationSubcategories = ['account', 'period', 'balanceSheet', 'incomeStatement'];
    for (const subcategory of dataValidationSubcategories) {
      if (!rules.dataValidationRules.hasOwnProperty(subcategory)) {
        return false;
      }
    }
    
    // Check dimensionalHierarchyRules subcategories
    const dimensionalHierarchySubcategories = ['entity', 'account'];
    for (const subcategory of dimensionalHierarchySubcategories) {
      if (!rules.dimensionalHierarchyRules.hasOwnProperty(subcategory)) {
        return false;
      }
    }
  ...
  /**
   * Check if a string can be safely converted to a function
   * @param {string} fnString - Function string to validate
   * @returns {boolean} Validation result
   */
  isValidFunctionString: function(fnString) {
    try {
      // Basic validation to prevent obvious security issues
      if (fnString.includes('eval(') || 
          fnString.includes('Function(') || 
          fnString.includes('setTimeout(') || 
          fnString.includes('setInterval(')) {
        return false;
      }
   ...
   * @param {string} fnString - Function string
   * @returns {function} Converted function
   */
  stringToFunction: function(fnString) {
    try {
  ...
  /**
   * Set up event listeners for the custom rules UI
   */
  setupEventListeners: function() {
    // Add event listeners for the custom rules UI elements
    const createRuleButton = document.getElementById('create-rule-btn');
    if (createRuleButton) {
      createRuleButton.addEventListener('click', this.showCreateRuleModal.bind(this));
    }
    
    const manageRulesButton = document.getElementById('manage-rules-btn');
    if (manageRulesButton) {
      manageRulesButton.addEventListener('click', this.showManageRulesModal.bind(this));
    }
    
    const exportRulesButton = document.getElementById('export-rules-btn');
    if (exportRulesButton) {
      exportRulesButton.addEventListener('click', this.handleExportRules.bind(this));
    }
    
    const importRulesButton = document.getElementById('import-rules-btn');
    if (importRulesButton) {
      importRulesButton.addEventListener('click', this.showImportRulesModal.bind(this));
    }
  },
  ...
  showCreateRuleModal: function() {
    ...
    if (modal) {
      ...
      if (form) {
        form.addEventListener('submit', this.handleCreateRule.bind(this));
      }
  ...
  /**
   * Handle category change in create rule form
   * @param {Event} event - Change event
   */
  handleCategoryChange: function(event) {
    const category = event.target.value;
    const subcategoryContainer = document.getElementById('rule-subcategory-container');
    const subcategorySelect = document.getElementById('rule-subcategory');
    ...
    if (subcategoryContainer &amp;&amp; subcategorySelect) {
      ...
      if (category === 'dataValidationRules') {
        subcategoryContainer.style.display = 'block';
        
        // Add options for data validation subcategories
        const subcategories = ['account', 'period', 'balanceSheet', 'incomeStatement'];
      ...
      } else if (category === 'dimensionalHierarchyRules') {
        subcategoryContainer.style.display = 'block';
        
        // Add options for dimensional hierarchy subcategories
        const subcategories = ['entity', 'account'];
        subcategories.forEach(subcategory =&gt; {
          const option = document.createElement('option');
          option.value = subcategory;
          option.textContent = this.formatSubcategoryName(subcategory);
    ...
    // Update validation function template based on category
    this.updateValidationTemplate(category);
  },
   ...
   * @param {string} category - Rule category
   */
  updateValidationTemplate: function(category) {
    ...
    if (validationTemplate &amp;&amp; conditionContainer &amp;&amp; conditionTemplate) {
      switch (category) {
        case 'namingRules':
          validationTemplate.textContent = 'function(name) {\n  // Return true if valid, false if invalid\n  return name.length &lt;= 80;\n}';
          conditionContainer.style.display = 'none';
          break;
          
        case 'dataValidationRules':
          conditionContainer.style.display = 'block';
          conditionTemplate.textContent = 'function(account, value) {\n  // Return true if this rule applies to this account/value\n  return account.startsWith("PL_");\n}';
          validationTemplate.textContent = 'function(value) {\n  // Return true if valid, false if invalid\n  return value &gt;= 0;\n}';
          break;
          
        case 'dimensionalHierarchyRules':
          validationTemplate.textContent = 'function(hierarchy) {\n  // Return true if hierarchy is valid, false if invalid\n  for (const member in hierarchy) {\n    // Check hierarchy rules\n  }\n  return true;\n}';
          conditionContainer.style.display = 'none';
          break;
          
        case 'intercompanyRules':
          validationTemplate.textContent = 'function(data) {\n  // Return true if intercompany data is valid, false if invalid\n  return true;\n}';
          conditionContainer.style.display = 'none';
          break;
          
        case 'movementRules':
          validationTemplate.textContent = 'function(data) {\n  // Return true if movement data is valid, false if invalid\n  return true;\n}';
          conditionContainer.style.display = 'none';
          break;
      }
    }
   ...
   */
  handleCreateRule: function(event) {
    event.preventDefault();
    
    const form = event.target;
    const category = form.elements['rule-category'].value;
    const subcategory = form.elements['rule-subcategory'].value;
    const name = form.elements['rule-name'].value;
    const message = form.elements['rule-message'].value;
    const validationFn = form.elements['rule-validation'].value;
    const conditionFn = form.elements['rule-condition']?.value;
    ...
    if (category === 'dataValidationRules' &amp;&amp; conditionFn) {
      rule.condition = conditionFn;
    }
    
    // Add rule
    const success = this.addRule(category, subcategory || null, rule);
    ...
    if (success) {
      alert(LanguageManager.translate('ruleCreatedSuccess'));
      
      // Refresh rules list if manage rules modal is open
      this.refreshRulesList();
    } else {
      alert(LanguageManager.translate('ruleCreatedError'));
    }
  },
  
  /**
   * Show the manage rules modal
   */
  showManageRulesModal: function() {
    const modal = document.getElementById('manage-rules-modal');
    if (modal) {
      // Show modal
      modal.style.display = 'block';
  ...
  /**
   * Populate the rules list in the manage rules modal
   */
  populateRulesList: function() {
    ...
    if (rulesList) {
      ...
      // Check if there are any custom rules
      let hasRules = false;
      
      // Process each category
      for (const category in allRules) {
        if (Array.isArray(allRules[category])) {
          // Category with direct rules array
          const rules = allRules[category];
          if (rules.length &gt; 0) {
            hasRules = true;
            this.addCategoryToRulesList(rulesList, category, null, rules);
          }
        } else {
          // Category with subcategories
          for (const subcategory in allRules[category]) {
            const rules = allRules[category][subcategory];
            if (rules.length &gt; 0) {
              hasRules = true;
              this.addCategoryToRulesList(rulesList, category, subcategory, rules);
            }
          }
        }
      }
      ...
      if (!hasRules) {
        const noRulesMessage = document.createElement('p');
        noRulesMessage.textContent = LanguageManager.translate('noCustomRules');
        noRulesMessage.className = 'no-rules-message';
        rulesList.appendChild(noRulesMessage);
      }
    }
  },
  
  /**
   * Add a category section to the rules list
   * @param {HTMLElement} rulesList - Rules list element
   * @param {string} category - Rule category
   * @param {string} subcategory - Rule subcategory
   * @param {array} rules - Rules array
   */
  addCategoryToRulesList: function(rulesList, category, subcategory, rules) {
    // Create category header
    const categoryHeader = document.createElement('h4');
    categoryHeader.className = 'rules-category-header';
    categoryHeader.textContent = this.formatCategoryName(category) + 
                                (subcategory ? ' - ' + this.formatSubcategoryName(subcategory) : '');
    rulesList.appendChild(categoryHeader);
    ...
    nameHeader.textContent = LanguageManager.translate('ruleName');
    headerRow.appendChild(nameHeader);
    
    const messageHeader = document.createElement('th');
    messageHeader.textContent = LanguageManager.translate('ruleMessage');
    headerRow.appendChild(messageHeader);
    
    const actionsHeader = document.createElement('th');
    actionsHeader.textContent = LanguageManager.translate('actions');
    ...
    rules.forEach(rule =&gt; {
      ...
      messageCell.textContent = rule.message;
      ...
      editButton.textContent = LanguageManager.translate('edit');
      editButton.addEventListener('click', () =&gt; {
        this.showEditRuleModal(category, subcategory, rule);
      });
      ...
      deleteButton.textContent = LanguageManager.translate('delete');
      deleteButton.addEventListener('click', () =&gt; {
        if (confirm(LanguageManager.translate('confirmDeleteRule'))) {
          this.deleteRule(category, subcategory, rule.id);
          this.refreshRulesList();
        }
      });
  ...
  /**
   * Show the edit rule modal
   * @param {string} category - Rule category
   * @param {string} subcategory - Rule subcategory
   * @param {object} rule - Rule to edit
   */
  showEditRuleModal: function(category, subcategory, rule) {
    const modal = document.getElementById('edit-rule-modal');
    if (modal) {
      // Set form values
      const form = document.getElementById('edit-rule-form');
      if (form) {
        form.elements['edit-rule-id'].value = rule.id;
        form.elements['edit-rule-category'].value = category;
        form.elements['edit-rule-subcategory'].value = subcategory || '';
        form.elements['edit-rule-name'].value = rule.name;
        form.elements['edit-rule-message'].value = rule.message;
        form.elements['edit-rule-validation'].value = typeof rule.validation === 'function' 
          ? rule.validation.toString() 
          : rule.validation;
        
        if (rule.condition) {
          const conditionContainer = document.getElementById('edit-condition-container');
          if (conditionContainer) {
            conditionContainer.style.display = 'block';
          }
          
          form.elements['edit-rule-condition'].value = typeof rule.condition === 'function'
      ...
      // Set up form submit handler
      if (form) {
        form.addEventListener('submit', this.handleEditRule.bind(this));
      }
   ...
   * @param {Event} event - Submit event
   */
  handleEditRule: function(event) {
    event.preventDefault();
    
    const form = event.target;
    const ruleId = form.elements['edit-rule-id'].value;
    const category = form.elements['edit-rule-category'].value;
    const subcategory = form.elements['edit-rule-subcategory'].value;
    const name = form.elements['edit-rule-name'].value;
    const message = form.elements['edit-rule-message'].value;
    const validationFn = form.elements['edit-rule-validation'].value;
    const conditionFn = form.elements['edit-rule-condition']?.value;
    ...
    // Add condition if applicable
    if (category === 'dataValidationRules' &amp;&amp; conditionFn) {
      updatedRule.condition = conditionFn;
    }
    
    // Update rule
    const success = this.updateRule(category, subcategory || null, ruleId, updatedRule);
    ...
    if (success) {
      alert(LanguageManager.translate('ruleUpdatedSuccess'));
      
      // Refresh rules list
      this.refreshRulesList();
    } else {
      alert(LanguageManager.translate('ruleUpdatedError'));
    }
  },
  ...
  refreshRulesList: function() {
    const manageRulesModal = document.getElementById('manage-rules-modal');
    if (manageRulesModal &amp;&amp; manageRulesModal.style.display === 'block') {
      this.populateRulesList();
    }
  },
  ...
  handleExportRules: function() {
    const rulesJson = this.exportRules();
    ...
    a.href = url;
    a.download = 'oracle_epm_custom_rules.json';
  ...
  /**
   * Show the import rules modal
   */
  showImportRulesModal: function() {
    const modal = document.getElementById('import-rules-modal');
    if (modal) {
      // Reset form
      const form = document.getElementById('import-rules-form');
      if (form) {
        form.reset();
      }
      ...
      // Set up form submit handler
      if (form) {
        form.addEventListener('submit', this.handleImportRules.bind(this));
      }
  ...
  /**
   * Handle import rules form submission
   * @param {Event} event - Submit event
   */
  handleImportRules: function(event) {
    event.preventDefault();
    
    const form = event.target;
    const fileInput = form.elements['import-rules-file'];
    
    if (fileInput.files.length === 0) {
      alert(LanguageManager.translate('noFileSelected'));
      return;
    }
    ...
    reader.onload = (e) =&gt; {
      const jsonRules = e.target.result;
      const success = this.importRules(jsonRules);
      ...
      if (success) {
        alert(LanguageManager.translate('rulesImportedSuccess'));
        
        // Refresh rules list
        this.refreshRulesList();
      } else {
        alert(LanguageManager.translate('rulesImportedError'));
      }
    };
  ...
  formatCategoryName: function(category) {
    switch (category) {
      case 'namingRules':
        return LanguageManager.translate('namingRulesTitle');
      case 'dataValidationRules':
        return LanguageManager.translate('dataValidationRulesTitle');
      case 'dimensionalHierarchyRules':
        return LanguageManager.translate('dimensionalHierarchyRulesTitle');
      case 'intercompanyRules':
        return LanguageManager.translate('intercompanyRulesTitle');
      case 'movementRules':
        return LanguageManager.translate('movementRulesTitle');
      default:
        return category;
    }
  },
   ...
   */
  formatSubcategoryName: function(subcategory) {
    switch (subcategory) {
      case 'account':
        return LanguageManager.translate('accountSubcategory');
      case 'period':
        return LanguageManager.translate('periodSubcategory');
      case 'balanceSheet':
        return LanguageManager.translate('balanceSheetSubcategory');
      case 'incomeStatement':
        return LanguageManager.translate('incomeStatementSubcategory');
      case 'entity':
        return LanguageManager.translate('entitySubcategory');
      default:
        return subcategory;
    }
  }
